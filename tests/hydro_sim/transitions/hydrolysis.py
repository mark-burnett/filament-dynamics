...
from hydro_sim import simulation_state

class RandomHydrolysisTest(unittest.TestCase):
    def setUp(self):
        self.strand = strand.Strand(['t', 'p', 'd'],
                                    ['d', 'd', 'p', 't', 'p', 't', 't'])
        self.sim_state = simulation_state.SimulationState(self.strand, None)
    ...
    def test_R(self):
        self.assertEqual(3, self.tp_transition.R(self.sim_state))
        self.assertEqual(1, self.pd_transition.R(self.sim_state))

    def test_perform(self):
        self.tp_transition.perform(None, self.sim_state, 1.5)
        for i, v in enumerate(['d', 'd', 'p', 't', 'p', 'p', 't']):
            self.assertEqual(v, self.strand[i])

        self.pd_transition.perform(None, self.sim_state, 0)
...
class PointedNeighborHydrolysisTest(unittest.TestCase):
    ...
    def test_R(self):
        self.assertEqual(2,   self.tp_transition.R(self.sim_state))
        self.assertEqual(0.5, self.pd_transition.R(self.sim_state))
