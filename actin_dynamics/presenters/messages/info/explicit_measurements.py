...
class ExplicitMeasurementList(object):
    __slots__ = ['data']
    def __init__(self, explicit_measurements):
        self.data = explicit_measurements

    @classmethod
    def from_simulation(cls, simulation):
        result = []
        for em in simulation.explicit_measurements:
            result.append((em.id, em.measurement_label.name, em.binding.class_name))

        result.sort(key=operator.itemgetter(1))

        return cls(result)

class ExplicitMeasurementInfo(object):
    def __init__(self, class_name=None, measurement_label=None,
    ...
    @classmethod
    def from_explicit_measurement(cls, em):
        pm = []
        for p in em.binding.parameter_mappings:
            pm.append((p.id, p.parameter_label.name, p.local_name))

        sm = []
        for s in em.binding.state_mappings:
            sm.append((s.id, s.state.name, s.local_name))

        return cls(class_name=em.binding.class_name,
                   measurement_label=em.measurement_label.name,
