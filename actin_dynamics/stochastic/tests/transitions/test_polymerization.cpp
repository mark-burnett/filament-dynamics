...
TEST_F(Polymerization, Basic) {
    transitions::BarbedEndPolymerization t_b0(zero, 1);
    transitions::BarbedEndPolymerization t_b1(one, 2);
    transitions::PointedEndPolymerization t_p0(zero, 3);
    transitions::PointedEndPolymerization t_p1(one, 4);
    ...
    EXPECT_EQ(6, concentrations[zero]->monomer_count());
    EXPECT_EQ(4, concentrations[one]->monomer_count());

    EXPECT_EQ(1, t_b0.perform(0, 8.5, filaments, concentrations));
    EXPECT_EQ(8, filaments[0]->length());
    EXPECT_EQ(9, filaments[1]->length());
    EXPECT_EQ(zero, filaments[1]->barbed_state());
    ...
    EXPECT_EQ(5, concentrations[zero]->monomer_count());
    EXPECT_EQ(3, concentrations[one]->monomer_count());

}

TEST_F(Polymerization, DisableTime) {
    transitions::BarbedEndPolymerization t_b0(zero, 1, 4);
    transitions::BarbedEndPolymerization t_b1(one, 2, 3);
    transitions::PointedEndPolymerization t_p0(zero, 3, 2);
    transitions::PointedEndPolymerization t_p1(one, 4, 1);
