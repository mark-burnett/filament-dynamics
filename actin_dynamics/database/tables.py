...
logging_table = schema.Table('logging', global_state.metadata,
        ...
        # Logger name
        schema.Column('name', types.String(MAX_NAME_LENGTH)),
        # Full path to file
        schema.Column('pathname', types.String(MAX_NAME_LENGTH)),
        # Name of originating function.
        schema.Column('funcName', types.String(MAX_NAME_LENGTH)),
        # Line number of logging event.
        schema.Column('lineno', types.Integer),
        # Logging level.
        schema.Column('levelno', types.Integer, index=True),
        schema.Column('levelname', types.String(MAX_POLY_LENGTH),
                      index=True),
        # User specified logging message.
        schema.Column('message', types.String(MAX_LOG_MESSAGE_SIZE)),
        mysql_engine='InnoDB')

exception_table = schema.Table('exception', global_state.metadata,
        schema.Column('logging_id', schema.ForeignKey('logging.id'),
                      primary_key=True),
        schema.Column('type_name',  types.String(MAX_NAME_LENGTH)),
        schema.Column('message', types.String(MAX_NAME_LENGTH)),
        mysql_engine='InnoDB')

traceback_table = schema.Table('traceback', global_state.metadata,
        schema.Column('id', types.Integer, primary_key=True),
        schema.Column('exception_id', types.Integer,
                      schema.ForeignKey('exception.logging_id'),
                      nullable=False),
        schema.Column('filename', types.String(MAX_NAME_LENGTH)),
        schema.Column('lineno', types.Integer),
        schema.Column('module', types.String(MAX_NAME_LENGTH)),
        schema.Column('line', types.String(MAX_NAME_LENGTH)),
        mysql_engine='InnoDB')
...
process_table = schema.Table('process', global_state.metadata,
        schema.Column('id', types.Integer, primary_key=True),
        schema.Column('type', types.String(MAX_POLY_LENGTH),
                      index=True),

        # These identify the code.
        schema.Column('code_revision', types.Integer),
        schema.Column('code_changeset', types.String(40)),

        # These identify the machine.
        # First, the hostname (not always identical to the below nodename).
        schema.Column('hostname', types.String(MAX_NAME_LENGTH)),
        # These 5 columns come from `uname -a`.
        schema.Column('sysname',  types.String(MAX_NAME_LENGTH)),
        schema.Column('nodename', types.String(MAX_NAME_LENGTH)),
        schema.Column('release',  types.String(MAX_NAME_LENGTH)),
        schema.Column('version',  types.String(MAX_NAME_LENGTH)),
        schema.Column('machine',  types.String(MAX_NAME_LENGTH)),

        # These identify the time.
        schema.Column('start_time', types.DateTime,
                      default=datetime.datetime.now),
        schema.Column('stop_time', types.DateTime),
        mysql_engine='InnoDB')
...
job_table = schema.Table('job', global_state.metadata,
        ...
        schema.Column('complete', types.Boolean, index=True,
                      default=False),
        mysql_engine='InnoDB')
...
fixed_argument_table = schema.Table('fixed_argument', global_state.metadata,
        schema.Column('argument_id', schema.ForeignKey('argument.id'),
                      primary_key=True),
        schema.Column('value', types.String(MAX_NAME_LENGTH)),
        mysql_engine='InnoDB')
...
bind_table = schema.Table('bind', global_state.metadata,
        schema.Column('id', types.Integer, primary_key=True),
        schema.Column('module_name',  types.String(MAX_POLY_LENGTH),
                      nullable=False, index=True),
        schema.Column('class_name', types.String(MAX_NAME_LENGTH),
                      nullable=False),
        schema.Column('label', types.String(MAX_NAME_LENGTH)),
        mysql_engine='InnoDB')
...
objective_data_table = schema.Table('objective_data_entry',
        ...
        schema.Column('objective_bind_id', schema.ForeignKey('bind.id'),
                      nullable=False),
        schema.Column('abscissa', types.Float),
        schema.Column('ordinate', types.Float),
        schema.Column('error',    types.Float),
        mysql_engine='InnoDB')
